...
const I18N = {
  zh: {
    ...
    nextWindow:'終場檔案', homeNote:'賽事已完結，首頁改成封存入口；完整理由、情報和紙上戰局收進細節。來源時間打架時先校時，不硬裝神。', detail:'展開細節', predict:'預測', paper:'紙上', intel:'情報',
  ...
  en: {
    ...
    nextWindow:'Final archive', homeNote:'The tournament is finished, so the homepage now works as an archive: predictions, intel, paper trades and post-match reviews stay in details for review.', detail:'Open details', predict:'prediction', paper:'paper', intel:'intel',
  ...
  const copy = lang === 'en'
    ...
    : archiveMode
      ? {
          ...
          body: '賽事已結束，首頁改先打開最近終場：預測、實際比分、公開情報與復盤線索都能點；之後只作為封存戰績留存。',
...
function PaperBankroll({ bankroll, matches = [], onSelect, lang, t }) {
  if (!bankroll) return null
  const pending = bankroll.pending || []
  const settled = bankroll.settled || []
  const last = settled.slice(-4).reverse()
  const matchByNumber = buildMatchByNumber(matches)
  const teamText = (team) => displayRouteTeam(team, matchByNumber)
  const pickText = (pick) => displayRoutePick(pick, lang, matchByNumber)
  return <section className="panel bankroll" id="paper-bankroll">
    ...
    {last.length > 0 && <div className="postmortem-block"><div className="mini-head"><b>{t.postmortem}</b><span>{t.clickReview}</span></div><div className="settled-ledger">{last.map((b) => <button type="button" key={b.matchId} onClick={() => onSelect(b.matchId)} className={b.status}><b>{teamText(b.team1)} vs {teamText(b.team2)}</b><span>{t.bet} {pickText(b.pick)} · {t.result} {b.score?.[0]}-{b.score?.[1]} · {resultText(b, lang)}</span><em>{marketLabel(b.marketReference, lang) || '1X2 paper market proxy'}</em></button>)}</div></div>}
    <div className="ledger">{pending.slice(0, 4).map((b) => <button type="button" key={b.matchId} onClick={() => onSelect(b.matchId)}><b>{teamText(b.team1)} vs {teamText(b.team2)}</b><span>{pickText(b.pick)} · {money(b.stake)} · {fmtDate(b.kickoffUtc, lang)}</span></button>)}</div>
  </section>
}
  ...
  const items = useMemo(() => {
    ...
    return matches
      ...
      .map((match) => {
        ...
        const displayTeam1 = resolveBracketRouteTeam(match.team1, matchByNumber)
        const displayTeam2 = resolveBracketRouteTeam(match.team2, matchByNumber)
        const p = prediction?.probabilities || {}
        const home = clamp01(p.home)
        const draw = clamp01(p.draw)
        const away = clamp01(p.away)
        const routeOnly = isRouteToken(displayTeam1) || isRouteToken(displayTeam2) || prediction?.pick === '待定'
        const favoriteIsHome = home >= away
        const favorite = favoriteIsHome ? displayTeam1 : displayTeam2
        const underdog = favoriteIsHome ? displayTeam2 : displayTeam1
...
function MatchDeepDive({ match, prediction, paperBet, intel, onClose, lang, t, nowMs, matchByNumber = {} }) {
  ...
  const p = prediction.probabilities
  const displayedPredictionPick = displayRoutePick(prediction.pick, lang, matchByNumber)
  const displayedPaperPick = paperBet ? displayRoutePick(paperBet.pick, lang, matchByNumber) : ''
...
function App() {
  ...
  return <main className="shell" lang={lang === 'en' ? 'en' : 'zh-Hant'}>
    ...
    <PaperBankroll bankroll={data.paperBankroll} matches={data.matches} onSelect={setSelectedId} lang={lang} t={t}/>
    ...
    {selectedDisplayMatch && <MatchDeepDive match={selectedDisplayMatch} prediction={data.predictions[selectedDisplayMatch.id]} paperBet={paperBetsByMatch[selectedDisplayMatch.id]} intel={intelByMatch[selectedDisplayMatch.id]} onClose={() => setSelectedId(null)} lang={lang} t={t} nowMs={nowMs} matchByNumber={matchByNumber}/>}<footer>{t.footer}</footer>
  </main>
}
export default App
