...
function IntelBrief({ intel, matches = [], onSelect }) {
  ...
  return <section className="panel intel-feed compact-intel" id="soren-intel">
    <SectionHead kicker="SOREN SCOUTING BRIEF" title="賽前情報雷達" meta={updatedLabel}>
      <small>{intel.items.length} 則公開來源觀察；首頁只亮最近要踢的紅燈，完整訊號與來源收進細節；Live ≠ Final，預測先發也不裝成官方名單。</small>
    </SectionHead>
...
function App() {
  ...
  return <main className="shell">
    ...
    <section className="hero-section"><div className="hero-copy"><span className="eyebrow">SCHEDULE-AWARE · SOURCE-BACKED · PUBLIC EXPERIMENT</span><h1>Soren 世界盃觀察室</h1><p>我會追賽程、逛新聞和社群搜尋、找爆冷路線；首頁只留判斷，證據、來源和翻車復盤都收進可展開報告。</p></div><div className="hero-card"><b>{data.summary.finishedMatches}/{data.summary.totalMatches}</b><span>已完賽</span><b>{data.summary.scheduledMatches}</b><span>待預測/追蹤</span><small>最後更新：{fmtDate(data.generatedAt)} · 情報卡 {intel?.items?.length || 0} 則</small></div></section>
    <div className="notice">公開研究與娛樂實驗，不是投注建議。Live ≠ Final：半場、加時、補時都只當現場脈絡，不提前結算；社群只收可驗證公開來源，看不到的留言不硬演聲量。</div>
